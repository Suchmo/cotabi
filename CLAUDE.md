...
Phase2のうち、プライバシーロック(WebAuthn/PINによる端末ロック)・思い出フラッシュバックは実装済み。タグ・カテゴリ付けは未着手。Phase3以降(ウィッシュリスト、検索・絞り込み、簡易費用記録、天気自動記録、動画対応、コメント・リアクション、OCR、プッシュ通知等)の実装には着手しない。フェーズの優先順位やスコープ判断に迷う場合はユーザーに確認する。

...
**Phase 2完了(プライバシーロック・思い出フラッシュバック・タグ付け すべて実装済み)。Phase3は未着手。**
...
タグ・カテゴリ付けは、タグを付ける・表示するところまでが対象で、タグでの検索・絞り込み(Phase3)は含まない。Phase3以降(ウィッシュリスト、検索・絞り込み、簡易費用記録、天気自動記録、動画対応、コメント・リアクション、OCR、プッシュ通知等)の実装には着手しない。フェーズの優先順位やスコープ判断に迷う場合はユーザーに確認する。

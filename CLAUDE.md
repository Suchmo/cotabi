...
Phase3のうち、ウィッシュリスト(`wishes`コレクション。TRIP/SPOTとは別管理で、訪問済みへの自動変換は行わない)は実装済み。検索・絞り込み・簡易費用記録は未着手。Phase4(天気自動記録、動画対応、コメント・リアクション、OCR、プッシュ通知等)の実装には着手しない。フェーズの優先順位やスコープ判断に迷う場合はユーザーに確認する。

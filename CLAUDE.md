...
**Phase 3完了(ウィッシュリスト・検索絞り込み・簡易費用記録 すべて実装済み)。Phase4は未着手。**

Phase1の機能スコープ(訪問マップ、旅行記録、データ共有、統計表示、アルバムまとめ、旅程表、ルート表示、PWA基本機能(ホーム画面追加・オフライン閲覧)など)は `docs/requirements_v5.md` の「2. 機能要件」および `docs/screen_design_v4.md` を参照。Phase2(プライバシーロック・思い出フラッシュバック・タグ付け)もすべて実装済み。タグ・カテゴリ付けは、タグを付ける・表示するところまでが対象で、タグでの検索・絞り込みはPhase3側で実装済み。

Phase3の内訳: ウィッシュリスト(`wishes`コレクション。TRIP/SPOTとは別管理で、訪問済みへの自動変換は行わない)、記録一覧の検索・絞り込み(日付範囲・国/都道府県・タグのAND条件)、簡易費用記録(TRIPの任意項目`costYen`。内訳管理はせず旅行全体の合計金額のみ、通貨は円のみ)。Phase4(天気自動記録、動画対応、コメント・リアクション、OCR、プッシュ通知等)の実装には着手しない。フェーズの優先順位やスコープ判断に迷う場合はユーザーに確認する。

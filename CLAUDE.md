...
**Phase 2は完了(プライバシーロック・思い出フラッシュバック・タグ付け すべて実装済み)。Phase3(旅の計画機能)実装中。**
...
タグ・カテゴリ付けは、タグを付ける・表示するところまでが対象で、タグでの検索・絞り込み(Phase3)は含まない。

Phase3のうち、ウィッシュリスト(`wishes`コレクション。TRIP/SPOTとは別管理で、訪問済みへの自動変換は行わない)・記録一覧の検索・絞り込み(日付範囲・国/都道府県・タグのAND条件)は実装済み。簡易費用記録は未着手。Phase4(天気自動記録、動画対応、コメント・リアクション、OCR、プッシュ通知等)の実装には着手しない。フェーズの優先順位やスコープ判断に迷う場合はユーザーに確認する。
